import { loadConfig, saveConfig, getConfigPath, getLogFile, getLogsDir, OpencodeRemoteConfig } from './config.js';
  ...
  <div class="container">
    ...
    <div class="card">
      ...
      <div class="network-info">
        <div class="info-box">
          <div class="label">Tailscale IP</div>
          <div class="value" id="tailscale-ip">Loading...</div>
        </div>
        <div class="info-box">
          <div class="label">Local IP</div>
          <div class="value" id="local-ip">Loading...</div>
        </div>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <form id="config-form">
        <div class="form-group">
          <label>OpenCode Web Port</label>
          <input type="number" id="opencode-web-port" min="1024" max="65535">
        </div>
        ...
        <div class="form-group">
          ...
          <input type="password" id="password" placeholder="Leave empty to disable">
        </div>
        
        <div class="form-group">
          <label>Hostname</label>
          <input type="text" id="hostname" placeholder="0.0.0.0">
        </div>
        
        <div class="form-group checkbox-group">
          <input type="checkbox" id="auto-start">
          <label style="margin: 0;">Auto-start on login</label>
        </div>
        
        <div class="form-group checkbox-group">
          <input type="checkbox" id="tailscale-only">
          <label style="margin: 0;">Show only Tailscale IP</label>
        </div>
        
        <div class="buttons">
          <button type="submit" class="btn btn-primary">Save</button>
          <button type="button" class="btn btn-secondary" onclick="restartService()">Restart Service</button>
        </div>
      </form>
    </div>
    ...
    <div class="card">
      ...
      <p style="color: #888; font-size: 13px; margin-bottom: 12px;">
        Config file: <code id="config-path"></code>
      </p>
  ...
  <script>
    ...
    async function loadService() {
      ...
      if (service.isActive) {
        ...
        statusEl.textContent = '✓ Service is running';
      } else {
        statusEl.className = 'status error';
        statusEl.textContent = '✗ Service is not running - Run: opencode-remote start';
      }
    }
    ...
    async function restartService() {
      ...
      const statusEl = document.getElementById('status');
      statusEl.className = 'status success';
      statusEl.textContent = '✓ Service restarted!';
